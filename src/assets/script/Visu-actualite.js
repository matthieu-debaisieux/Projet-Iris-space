const STORAGE_KEY = "iris_actualites";
      ...
      function openActuModal(actu) {
        ...
        document.getElementById('modal-meta').textContent    = actu.date + ' — Par ' + actu.auteur;
        document.getElementById('modal-contenu').textContent = actu.contenu;
        document.getElementById('modal-date').textContent    = 'Publié le ' + actu.date;

        const modal = document.getElementById('actu-modal');
        modal.style.opacity       = '1';
        modal.style.pointerEvents = 'all';
        document.getElementById('actu-modal-box').style.transform = 'translateY(0)';
      ...
      function closeActuModal() {
        ...
        document.getElementById('actu-modal-box').style.transform = 'translateY(24px)';
      ...
      document.addEventListener("DOMContentLoaded", () => {
        ...
        publies.forEach(actu => {
          ...
          card.className = "max-w-md bg-gray-200 rounded-3xl p-5 border border-black/[0.08] flex flex-col gap-4 hover:-translate-y-1 transition-transform duration-200";
